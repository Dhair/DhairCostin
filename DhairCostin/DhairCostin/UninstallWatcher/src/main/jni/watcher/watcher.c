...
int main(int argc, char *argv[])
{
	...
	__android_log_print(ANDROID_LOG_ERROR, "tag", "Copyright (c) 2015, Vincent Cheung<coolingfall@gmail.com>:%d", 0);
	...
	else if (pid == 0)
	{
		...
		LOGD(LOG_TAG, "watcher process fork ok, start to watch watch");
		while (1)
		{
			LOGD(LOG_TAG, "watcher process fork ing one ");
			/* read will block process */
			size_t read_bytes = read(fd, p_buf, sizeof(struct inotify_event));
			LOGD(LOG_TAG, "watcher process fork ing second ");
			/* delay 200ms */
			usleep(200*1000);
			LOGD(LOG_TAG, "watcher process fork ing 2");
